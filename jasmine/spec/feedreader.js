...
$(function() {
    ...
    describe('The menu', function() {
         ...
         */
         it('the menu changes visibility when menu icon is clicked', function() {
            // defines variable we need to check when clicked 
            var visibilityCheck = $('.menu-icon-link');
            // checks if menu displays when clicked once
            visibilityCheck.click();
            expect($('body').hasClass('menu-hidden')).toBe(false);
            // checks if menu hides when clicked twice
            visibilityCheck.click();
            expect($('body').hasClass('menu-hidden')).toBe(true);
          });
        });
    ...
    describe('New Feed Selection', function() {
        ...
        it('when a new feed is loaded by the loadFeed function the content changes', function(done) {
            loadFeed(1, function() {
                loadFeedTwo = $('.feed').html();
                expect(loadFeedTwo).not.toEqual(loadFeedOne);
                done();
            });
        });
    });
